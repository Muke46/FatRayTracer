...
bool Sphere::Intersect(const Ray3& ray, Ray3& reflection) const
{
    ...
    if (tc < 0.0f) return false; // The sphere is behind the ray
    ...
    // Generate a random reflection ray (rough objects)

    // // Generate a random Vector
    // Vector3 direction = Vector3(distribution(generator), distribution(generator), distribution(generator));
